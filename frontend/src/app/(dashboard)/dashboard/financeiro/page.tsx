...
// um modelo treinado.
function CollectionsSuggestions({
  collections,
}: {
  collections: AiCollectionsSuggestionsData;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="font-medium text-slate-900">
          Cobranças Inteligentes ({collections.count})
        </h2>
        <p className="mt-1 text-sm text-slate-500">{collections.narrative}</p>
      </div>
      <table className="w-full text-left text-sm">
        <thead className="text-slate-500">
          <tr>
            ...
            <th className="px-4 py-2 font-medium">Canal sugerido</th>
          </tr>
        </thead>
        <tbody>
          {collections.suggestions.map((s) => (
            <tr key={s.invoiceId} className="border-t border-slate-100">
              <td className="px-4 py-2">{s.customerName}</td>
              <td className="px-4 py-2">R$ {formatMoney(s.remaining)}</td>
              <td className="px-4 py-2">{s.daysOverdue}d</td>
              <td className="px-4 py-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${PRIORITY_BADGE[s.priority] ?? 'bg-slate-100 text-slate-600'}`}
                >
                  {s.priority}
                </span>
              </td>
          ...
          {collections.suggestions.length === 0 && (
            <tr>
              <td colSpan={5} className="px-4 py-6 text-center text-slate-500">
                Nenhuma sugestão de cobrança no momento.
              </td>
            </tr>
          )}
        </tbody>
