...
export class FinanceService {
   ...
   * classificando em verde/amarelo/vermelho.
   *
   * SUPERSEDIDO na Fase 4 pelo "Score IA" (`AiService.scoreIa`, endpoint
   * `GET /ai/customers/:id/score-ia`), que é o que o Cliente 360° passou a
   * exibir como score principal — ver docs/fase4-ia-corporativa.md para a
   * justificativa da troca. Este método continua existindo e não foi
   * removido por dois motivos: (1) `AiService.scoreIa` o chama e reusa
   * `punctualityRate`/`delinquencyRate`/`relationshipDays` como um dos seis
   * sinais do modelo ponderado (decisão deliberada — em vez de duplicar essa
   * conta, ver AiService.scoreIa); (2) o endpoint `GET /finance/customers/:id/score`
   * continua respondendo, para não quebrar nenhum consumidor externo que já
   * dependa dele. Documentado aqui e em docs/fase4-ia-corporativa.md como
   * "ainda determinístico, mas agora um sinal de entrada, não a resposta
