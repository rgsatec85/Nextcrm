...
## `ai_query_logs` (Fase 4): mesma RLS, sem regra de acesso nova

A única tabela nova da Fase 4 (`ai_query_logs`, migration
`0005_fase4_ia.sql`) segue exatamente o mesmo padrão de tenant_id + RLS
(`USING (tenant_id = current_tenant_id())`) das tabelas anteriores — não
introduz nenhuma regra de isolamento nova. O `AiModule` inteiro (`/ai/*`) é
restrito a `admin`/`gestor`/`vendedor`/`financeiro` (nunca `cliente_portal`)
e reusa o ABAC já existente dos módulos que consulta
(`CustomersService`/`OpportunitiesService`/`InvoicesService`/
`ContractsService`) em vez de implementar uma checagem de acesso própria —
por exemplo, `GET /ai/customers/:id/summary` só chega aos dados de um
cliente se `CustomersService.findOne` (que já aplica `assertOwnership`)
permitir. RLS reconfirmada ao vivo num Postgres 16 real com dois tenants
simulados, mesma metodologia de sempre — detalhes em
