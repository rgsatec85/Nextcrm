...
fundação (Fase 0); o CRM Comercial (Fase 1), o Módulo Financeiro (Fase 2), o
Portal do Cliente/Atendimento (Fase 3) e a Inteligência Artificial Corporativa
(Fase 4) reaproveitam tudo isso sem mudanças estruturais — detalhes
específicos em `docs/fase1-crm-comercial.md`, `docs/fase2-financeiro.md`,
`docs/fase3-portal-atendimento.md` e `docs/fase4-ia-corporativa.md`.
...
### 10. `AiProvider`: abstração de IA trocável, sem integração real de LLM/ML no sandbox (Fase 4)

Este sandbox não tem GPU nem um servidor Ollama alcançável, e não existe
pipeline de treinamento nem dataset rotulado no projeto. Em vez de fingir uma
integração real, a Fase 4 introduz uma interface mínima
(`backend/src/modules/ai/providers/ai-provider.interface.ts`):
`generateText(prompt, context) => Promise<string>`. Quem chama sempre
computou os números/fatos reais do tenant antes (nunca inventados) e passa
isso em `context`; o provider só decide **como** fraseá-los, nunca **quais**
números aparecem — isso vale tanto para o `DeterministicAiProvider` (default,
sem rede, templates fixos em `ai-templates.ts`, único caminho testado) quanto
para o `OllamaAiProvider` (real, faz `fetch` para um servidor Ollama
self-hosted, mas não exercitado pelos testes por falta de rede no sandbox).
Seleção via `AI_PROVIDER=deterministic|ollama` (default `deterministic`),
resolvida por uma factory provider do Nest injetada por `ConfigService` em
`AiModule` — nenhum consumidor (`AiService`) conhece a implementação
concreta por trás do token `AI_PROVIDER`. Uma falha do `OllamaAiProvider`
(timeout, HTTP não-ok, resposta vazia) nunca é lançada — cai de volta para o
`DeterministicAiProvider`, mesma filosofia de confiabilidade do
`WebhooksService.dispatch()` (decisão #9). Mesma classe de "simplificação
documentada" já usada para o gateway de pagamento fake (Fase 2, decisão em
`docs/fase2-financeiro.md`) e para o fire-and-forget de webhooks sem fila de
retry (decisão #9 abaixo) — ver `docs/fase4-ia-corporativa.md` para o
